...
style_filepath = {}
style_name_list = []
weight_path = glob.glob("./transform_weight/*.pth")
    ...
    while True:
        ...
        if style_choice.isnumeric():
            ...
            if not os.path.exists(content_img_path):
                ...
                break
            content_image = utils.loadImage(content_img_path)
            starttime = time.time()
            content_tensor = utils.imgToTensor(content_image).to(device)

            STYLE_TRANSFORM_PATH = os.path.abspath(os.path.join(os.path.dirname( __file__ ), style_filepath[style_name_list[style_choice]]))
            ...
            generated_image = utils.tensorToImg(generated_tensor.detach())
            save_path = './images/stylized/' + 'shanshui_' + content_img_path.split('\\')[-1]
            print (save_path)
